(function() {
    ...
    setting.$inject = ['$scope', '$state', '$timeout', '$location', 'settingServices', 'loginRegiServices', 'manageadServices', '$filter', 'editionPayment', 'mProfileServices', 'adDataServices', 'EventServices', 'TrainingServices'];
    function setting($scope, $state, $timeout, $location, settingServices, loginRegiServices, manageadServices, $filter, editionPayment, mProfileServices, adDataServices, EventServices, TrainingServices) {
...
//            $state.go('setting');
        ...
        console.log("allDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallData");
        console.log(allData);
        console.log("allDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallDataallData");
        $('.add_new_event').hide();
        $('.edit_new_event').hide();
...
//        }
        ...
        function planselect(id) {
            ...
            {
                vm.plan_type = "PREMIUM";

            } else if (id == "2")
            {
                vm.plan_type == "FEATURED";
            }
...
//                vm.checkfeature = true;
                ...
                vm.showAd_amnt = vm.premium_price;
                vm.showMulti = true;
...
//                vm.checkfeature = true;
                ...
                vm.showAd_amnt = vm.featured_price;
                vm.showMulti = true;
            }
            else {
                vm.showMulti = false;
...
//            alert(vm.eventObj.plan_id);
            ...
            if (id == "0")
            {
                vm.event_plan_type = "FREE";
            } else if (id == "1")
            {
                vm.event_plan_type = "PREMIUM";
            } else if (id == "2")
            {
                vm.event_plan_type = "FEATURED";
            }
        }
...
//                vm.checkfeature = true;
                ...
                vm.showEvent_amnt = vm.premium_price;
                vm.showMulti = true;
...
//                vm.checkfeature = true;
                ...
                vm.showEvent_amnt = vm.featured_price;
                vm.showMulti = true;
            }
            else {
                vm.showMulti = false;
...
//            vm.adObj.images = imgId.toString();
            ...
            if (vm.eventObj.total_payable == 0)
            {
                console.log('0 pay');
                var data = JSON.parse(localStorage.getItem('eventObject'));
                vm.title = data.event_tittle;
...
//        View Stats
        vm.total_ad_count_data = "";
        vm.total_view_count_data = "";
        vm.viewStatData = {
            'user_id': localStorage.getItem("user_id"),
        }
        settingServices.showStats(vm.viewStatData).success(function(response) {
            console.log("in stats view data");
            console.log(response);
            if (response.status == 105) {
//                    showSuccess("Password updated successfully..");
                // if (response.data.length > 0) {
                vm.total_ad_count_data = response.data['total_ad_count'];
                vm.total_view_count_data = response.data['total_views_count'];
...
//            localStorage.setItem('category_name', cat_name);
            ...
            manageadServices.viewsubcategory(vm.subcatData).success(function(response) {
                ...
                if (response.status == 105) {
                    ...
                    $('.ad_navigation').css('display', 'block');
                    vm.showLoading = true;
                }
                else {
                    showError(response['message']);
...
//            })
            ...
            } else
            {
                ...
                //editionPayment.payment.checkout('PayPal', vm.adObj.total_payable, "http://localhost:8888/epardesh-phase2/epardesh/web/#/setting");
...
//            console.log(vm.TrainingObj.training_plan_id);
            ...
            if (id == "0")
            {
                vm.training_plan_type = "FREE";
            } else if (id == "1")
            {
                vm.training_plan_type = "PREMIUM";
            } else if (id == "2")
            {
                vm.training_plan_type = "FEATURED";
            }
        }
...
//            vm.trim = vm.despre.trim();
            ...
            if (vm.training_trim.length >= '100' && vm.training_trim.length <= '8000')
            {

            } else {
                showError('Description Should be in between 100 to 8000');
                $timeout(function() {
                }, 1000);
                return false;
            }
...
//            vm.adObj.images = imgId.toString();
            ...
            if (vm.reccured == true)
            {
                ...
                if (vm.TrainingObj.total_payable == 0)
                {
                    var data = JSON.parse(localStorage.getItem('trainingObject'));
                    vm.training_title = data.training_tittle;
                    vm.training_description = data.training_description;
...
//                editionPayment.payment.checkout('PayPal', vm.TrainingObj.total_payable, "http://localhost:8888/epardesh-web/web/#/setting");
                    editionPayment.payment.checkoutrecur('PayPal', vm.TrainingObj.total_payable, '10', vm.TrainingObj.training_tittle, "http://www.epardesh.com/setting");
...
//            $('#editTraining_Contact_number').val(training_phone);
        ...
        function updateTrainingFun()
        {
            ...
            if (vm.edittraining_despre.length >= '100' && vm.edittraining_despre.length <= '8000')
            {
...
//                    }, 3000);vm.doPayment
        ...
        //====get today date to show pre-filled===============
        var d = new Date();
       ...
       //event
        ...
        $('#selectstartdate').val(new_date);
        $('#selectenddate').val(new_date);
        ...
        //training
        $('#trainingstartdate').val(new_date);
        $('#trainingenddate').val(new_date);
