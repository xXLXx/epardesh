(function() {
    'use strict';
    var controllerId = 'SearchTrainingCtrl';
    angular
            .module('app')
            .controller(controllerId, SearchTrainingCtrl);
    SearchTrainingCtrl.$inject = ['$scope', '$auth', '$state', '$sessionStorage', '$timeout', 'TrainingServices', 'loginRegiServices', 'manageadServices'];
    function SearchTrainingCtrl($scope, $auth, $state, $sessionStorage, $timeout, TrainingServices, loginRegiServices, manageadServices) {
...
//                vm.allcategory.concat(vm.subcategory);
        ...
        var searchData = JSON.parse(localStorage.getItem('TrainingData'));
        console.log('data');
        console.log(searchData);
        vm.upcomingEvent = '';
        $scope.$on("reloadtraining", function(event) {
            $state.reload();
        });
            ...
            TrainingServices.contactTraining(vm.viewTrainingCon).success(function(response) {
                ...
                if (response.status == 105) {
                    ...
                    $timeout(function() {
                        ...
                        vm.msg = "";
                        vm.viewTrainingCon.sender_name = '';
                        vm.viewEventCon.sender_email = '';
                        vm.viewEventCon.sender_phone = '';
...
//                        $('.ad_fav').html('added');
        ...
        $scope.makeTodos = function() {
            ...
            for (var i = 0; i < searchData.trainings.length; i++) {
                // $scope.todos.push(allData.ads[i]);
                $scope.todos.push(searchData.trainings[i]);
            }
        };
        $scope.makeTodos();

        $scope.numPages = function() {
            return Math.ceil(searchData.trainings.length / $scope.numPerPage);
        };
