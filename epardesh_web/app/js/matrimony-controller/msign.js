(function() {
    ...
    mSignCtrl.$inject = ['$scope', '$state', '$timeout', '$http', '$location', 'loginRegiServices', 'mLoginRegiServices', 'editionPayment', 'matrimonypromocode'];
    function mSignCtrl($scope, $state, $timeout, $http, $location, loginRegiServices, mLoginRegiServices, editionPayment, matrimonypromocode) {
        ...
        function getValidity(index) {
            ...
            if (index == "0")
            {
                vm.validity_show = "1 MONTH";
                vm.validity= 0;

            } else if (index == "1")
            {
                vm.validity_show = "3 MONTHS";
                vm.validity= 1;

            } else if (index == "2")
            {
                vm.validity_show = "6 MONTHS";
                vm.validity= 2;
            }
        ...
        function planselect(status) {
            ...
            console.log("vm.freeArr array plan");
...
//                alert('register data');
        ...
        function payNow() {
            ...
            if (vm.btnTitle != 'Submit') {
                ...
                if (vm.price == 0)
                {
                    console.log('inside payment 0 price')
                    vm.registerNow();
...
//                editionPayment.payment.checkout('PayPal', vm.price, "http://54.172.109.78/epardesh/epardesh-75way/web/#/msign");
                //editionPayment.payment.checkout('PayPal', vm.price, "http://localhost:8888/epardesh-phase2/epardesh/web/#/msign");
        ...
        function registerNow() {
            ...
            if (vm.signObj.country == "India" || vm.signObj.country == "INDIA" || vm.signObj.country == "india" || vm.signObj.country == "IN" || vm.signObj.country == "in") {
                vm.signObj.mobile = "+91" + vm.signObj.mobile;
                vm.signObj.alternate_mobile = "+91" + vm.signObj.alternate_mobile;
            }
            else {
                vm.signObj.mobile = "+1" + vm.signObj.mobile;
                vm.signObj.alternate_mobile = "+1" + vm.signObj.alternate_mobile;
            }
...
//               console.log(index+'rfdsgfvdh');
        ...
        function cancel()
        {
            ...
            vm.payment = false;
            vm.plan = true;
            $('.newad_menu').addClass('active_menu');
            $('.register').removeClass('active_menu');


        }

//        $(".pac-item").on("click", function(){
//            console.log("From delegate on select click");
//            console.log($("#g-address").val());
//        });

//$("body").live("click", ".pac-item", function(){
//    console.log("From delegate on select click");
//    console.log($("#g-address").val());
//});


        //console.log("From delegate on select clickFrom delegate on select clickFrom delegate on select clickFrom delegate on select click");


//    $('body').on('blur', '#g-address', function () {
//        console.log("From delegate on select blur");
//    console.log($("#g-address").val());
//    });
...
//    });
        ...
        function addressFunc() {
            setTimeout(function() {
                var address = vm.signObj.address;
                console.log(address);
                console.log($("#g-address").val());
                if (typeof address === "undefined") {
                    ...
                    $http.get("http://maps.googleapis.com/maps/api/geocode/json?address='" + vm.signObj.address + "'")
                            .success(function(response) {
                        console.log("response");
                        console.log(response);
                        var getAddressComponents = response.results[0]['address_components'];
                        var getFormattedAddress = response.results[0]['formatted_address'];
                        vm.signObj.address = getFormattedAddress;
                        for (var i = 0; i < getAddressComponents.length; i++) {
                            var value_type = response.results[0]['address_components'][i]["types"][0];
                            if (value_type == "country") {
                                vm.signObj.country = response.results[0]['address_components'][i]["long_name"];
                            }
                            else if (value_type == "administrative_area_level_1") {
                                vm.signObj.state = response.results[0]['address_components'][i]["long_name"];
                            }
                            else if (value_type == "locality") {
                                vm.signObj.city = response.results[0]['address_components'][i]["long_name"];
                            }
